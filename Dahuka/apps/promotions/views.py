...
def promotion_list(request):
    """
    View hiển thị danh sách khuyến mãi với bộ lọc và thống kê (Skinny View).
    """
    query = request.GET.get('q', '')
    status = request.GET.get('status', '')
    
    # Lấy thống kê từ Selector
    stats = PromotionSelector.get_promotion_stats()
    
    # Lấy danh sách đã lọc từ Selector
    promotions_list = PromotionSelector.get_filtered_promotions(query=query, status=status)
...
def add_promotion(request, pk=None):
    """
    View thêm/sửa khuyến mãi và kích hoạt gửi thông báo qua Service.
    """
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Giao phó việc gửi thông báo cho Service Layer
...
def api_promotion_detail(request, pk):
    """
    API trả về chi tiết khuyến mãi (Sử dụng cho Frontend/Modals).
    """
    promotion = get_object_or_404(Promotion, pk=pk)
